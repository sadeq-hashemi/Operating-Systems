...
check_extralink() {
  ...
  uint extra_reference = 0;
  struct dinode *i, *inner ;
  uint ind, inum;
  uint *indirect_block_addr;
  struct dirent *entry, *addr, *indirect_addr;
 	 
  for(i = ntable; i < &ntable[sb->ninodes]; i ++) {
    if(i->type != T_DIR) // get the directories
      continue;
    extra_reference = 0;
    inum = i - ntable;

    for(inner = ntable; inner < &ntable[sb->ninodes]; inner++) { //check inodes for other directories
      if(inner->type != T_DIR || inner - ntable == inum) {
        continue;
      }
      for(ind = 0; ind <NDIRECT; ind++) {
        if(inner->addrs[ind] == 0) 
          break;
        addr = (struct dirent *)getAddr(inner->addrs[ind]);
        if(addr == NULL)  {
          ERROR("bad direct address in inode");
          }
        for(entry = addr; entry < &addr[NUM_ENTRIES]; entry++) {
        ...
        if(indirect_block_addr == NULL)  {
          ...
          indirect_addr = (struct dirent *)getAddr(indirect_block_addr[ind]);
            ...
            for(entry = indirect_addr; entry < &indirect_addr[NUM_ENTRIES] ; entry++) {
              if(strcmp(entry->name, ".") == 0 || strcmp(entry->name, "..") == 0)
                continue;
              if(entry->inum  == inum){
                extra_reference++;
...
*/
   ...
   if(entry[1].inum != 1 && strcmp(entry[1].name, "..") != 0)
    // printf("root2");
     ERROR("directory not properly formatted");
   if(entry->inum != 1 || entry[1].inum != 1)
     ERROR("root directory does not exist");
...
//DIRECT
 ...
 //             ERROR("parent directory mismatch");
            ...
            if((entry->inum == 0) ||  (strcmp((entry->name), "..")) != 0)
             // printf("else check reg dir check parent ");
              ERROR("directory not properly formatted");
            temp_inode = &ntable[entry->inum];
            if(entry->inum == inum ||
               (temp_inode->type != T_DIR) || 
                 (check_parent(temp_inode, inum) < 0))              
              ERROR("parent directory mismatch");
          }
          }
        }
