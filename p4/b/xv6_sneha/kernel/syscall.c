...
isValid(struct proc* p, uint addr, uint n) {
  int valid =
     ( (addr == 0x0) || ((addr+n <= p->sz) && (addr+n >= USERBOT) && (addr <=p->sz) && (addr >= USERBOT)) || ((addr+n < USERTOP) && (addr <= USERTOP) && (addr >= p->stack_sz-PGSIZE) && (addr+n <= p->stack_sz + PGSIZE)&& (addr <= p->stack_sz + PGSIZE ) && (addr + n >= p->stack_sz - PGSIZE))); 
  return valid;  
}
