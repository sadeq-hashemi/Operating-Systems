#include "xv6_fsck.h"
int
check_extralink(struct dinode *ip, uint inum) {
...
check_reg_dir(struct dinode* ip, uint inum) {
 ...
 uint temp, bmap_entry;
 uint* indaddr;
 uint* addrs, *addrs_temp;
 uint inum_entry;
 struct dirent  *self, *parent, *dirent, *dirent_start, *entry, *temp_dirent;
 struct dinode* temp_inode;
 
  root_found = check_root(ip, inum);
