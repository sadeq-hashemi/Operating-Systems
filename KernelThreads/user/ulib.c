...
sem_init(sem_t *sem, int n) {
  sem->count = n;
  lock_init(&(sem->sem_lock));
  cond_init(&(sem->sem_cond));
}

void
sem_wait(sem_t *sem) {
   lock_acquire(&(sem->sem_lock));
   while(sem->count <= 0)
     cond_wait((void*)&(sem->sem_cond), (void*)&(sem->sem_lock));
   sem->count = sem->count - 1;
   lock_release(&(sem->sem_lock));
