...
{
  if( (addr >= p->sz && addr < p->stack_sz) || (addr+4 > p->sz && addr+4<p->stack_sz) || addr < 0x2000)
    return -1;
  *ip = *(int*)(addr);
  return 0;
}
...
fetchstr(struct proc *p, uint addr, char **pp)
{
  ...
  if( (addr >= p->sz && addr < p->stack_sz) || addr < 0x2000)
    return -1;
  *pp = (char*)addr;
  if(addr > p->sz) {
  ep = (char*)USERTOP;
...
argptr(int n, char **pp, int size)
{
  ...
  if(argint(n, &i) < 0)
    return -1;
 if ( ( (uint) i >= proc->sz &&  (uint)i < proc->stack_sz ) || ( (uint)i+size > proc->sz && (uint) i+size < proc->stack_sz ) || ((uint)i < 0x2000)  )
    return -1;
  *pp = (char*)i;
  return 0;
}
