...
// when a CPU is not running any process (kpgdir).
	// A user process usesu the same page table as the kernel; the
...
allocuvm_stack(pde_t *pgdir, uint oldsz, uint newsz)
{
  ...
  if(newsz > USERTOP)
    return 0;
  if(newsz > oldsz)  //new cond
    return oldsz;

  a =(uint) PGROUNDDOWN(oldsz - 1); //down
  for(; a > newsz; a -= PGSIZE){ // > , -=
    mem = kalloc();
    if(mem == 0){
      cprintf("allocuvm_stack out of memory\n");
      deallocuvm_stack(pgdir, newsz, oldsz);  //update for stack
      return 0;
    }
    memset(mem, 0, PGSIZE);
    mappages(pgdir, (char*)a, PGSIZE, PADDR(mem), PTE_W|PTE_U);
...
// process size.  Returns the new process size.
int
deallocuvm_stack(pde_t *pgdir, uint oldsz, uint newsz)
{
  pte_t *pte;
  uint a, pa;
